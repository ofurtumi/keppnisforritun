...
{
	if (y < 0)
		return -INF; // Við erum búin að setja of mikið í pokann.
	if (x < 0)
		return 0; // Engir hlutir eftir.
	if (d[x][y] != -1)
		return d[x][y];						  // Við erum búin að reikna svarið. Skilum því.
...
void knapsack(int *v, int *w, int *r, int n, int c) // Leysir bakpokadæmið. Fylkið r segir hvaða hluti á að taka.
{
	...
	for (i = 0; i < n; i++)
		for (j = 0; j <= c; j++)
			d[i][j] = -1; // Upphafstillum minnistöfluna.
	for (i = 0; i < n; i++)
		a[i] = v[i], b[i] = w[i], r[i] = 0;							 // Upphafstillum restina.
...
int main()
{
	...
	for (c = 0; c < 30; c++)
	{
		...
		if (scanf("%d%d", &c, &n) == 1)
		{
			printf("scannar");
			for (i = 0; i < n; i++)
				scanf("%d%d", &v[i], &w[i]); // Innlestri lokið.
			knapsack(v, w, r, n, c);		 // Reiknum svarið.
			for (k = i = 0; i < n; i++)
				k += r[i];	   // Teljum hvað við völdum marga hluti.
			printf("%d\n", k); // Prentum hvað við völdum marga hluti.
			for (i = 0; i < n; i++)
				if (r[i])
					printf("%d ", i); // Prentum hlutina sem við völdum.
			printf("\n");
		} // Innlestur hefst.
		else
			break;
