...
fn main() {
    let mut lines: Vec<Vec<usize>> = io::stdin()
        .lock()
        .lines()
        .map(|l| {
            l.unwrap()
    ...
    let tests = lines.remove(0)[0];
    for _i in 0..tests {
        let header = lines.remove(0);
        let e_num = header[0];
        let v_num = header[1];
        let mut nodes: Vec<Vec<u32>> = vec![Vec::new(); e_num];

        let mut counter = 0;
        while counter < v_num {
            let edge = lines.remove(0);
            let (i, j) = (edge[0] - 1, edge[1] - 1);
            nodes[i].push(j as u32);
            nodes[j].push(i as u32);
            counter += 1;
        }

        let g: Graph = nodes;
        let output = bfs(g, 0, (e_num - 1) as u32).unwrap();
        println!("{:?}", output)
    }
}
